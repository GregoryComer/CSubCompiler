...
namespace CSubCompiler.AST.BinaryOperators
{
    ...
    {
        protected BooleanBinaryOperatorNode(SubExpressionNode left, SubExpressionNode right, Token token, int tokenIndex) : base(left, right, token, tokenIndex)
        {
        ...
        public override ILType GetResultType(ILGenerationContext context)
        {
            ...
            if (leftType.Category == ILTypeCategory.Pointer || rightType.Category == ILTypeCategory.Pointer)
            {
